...
if [ ${INPUT_MULTI_BINARIES^^} == 'TRUE' ]; then
  ...
  GOAMD64=${GOAMD64_FLAG} GOARM=${GOARM_FLAG} GOOS=${INPUT_GOOS} GOARCH=${INPUT_GOARCH} ${INPUT_BUILD_COMMAND} ${INPUT_BUILD_FLAGS} --output ${BUILD_ARTIFACTS_FOLDER} ${INPUT_PROJECT_PATH} 
else
  ...
  if [[ "${INPUT_BUILD_COMMAND}" =~ ^make.* ]]; then
    ...
    GOAMD64=${GOAMD64_FLAG} GOARM=${GOARM_FLAG} GOOS=${INPUT_GOOS} GOARCH=${INPUT_GOARCH} ${INPUT_BUILD_COMMAND} --output ${BUILD_ARTIFACTS_FOLDER}/${BINARY_NAME}${EXT} ${INPUT_BUILD_FLAGS} ${LDFLAGS_PREFIX} "${INPUT_LDFLAGS}"
  fi
fi
